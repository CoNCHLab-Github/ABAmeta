...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ali/Workspace/ABAmeta/experiment/ABA_pilot_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    # create speaker 'trial_ABA_sequence'
    deviceManager.addDevice(
        deviceName='trial_ABA_sequence',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trial_fixation_cross = visual.ShapeStim(
        win=win, name='trial_fixation_cross', vertices='cross',
        size=(0.05, 0.05),
        ori=0.0, pos=(0, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor=[-1.0000, -1.0000, -1.0000],
        opacity=None, depth=0.0, interpolate=True)
    trial_ABA_sequence = sound.Sound(
        'A', 
        secs=-1, 
        stereo=True, 
        hamming=True, 
        speaker='trial_ABA_sequence',    name='trial_ABA_sequence'
    )
    trial_ABA_sequence.setVolume(0.5)
    trial_key_resp = keyboard.Keyboard(deviceName='trial_key_resp')
    # Run 'Begin Experiment' code from trial_multiple_keypress
    ...
    for thisTrial in trials:
        ...
        trial_ABA_sequence.setSound(stimuli, secs=7.0, hamming=True)
        trial_ABA_sequence.setVolume(0.5, log=False)
        trial_ABA_sequence.seek(0)
        # create starting attributes for trial_key_resp
        trial_key_resp.keys = []
        trial_key_resp.rt = []
        _trial_key_resp_allKeys = []
        # Run 'Begin Routine' code from trial_multiple_keypress
        # Begin Routine tab
        ...
        response_times = []  # Reset for each trial
        press_durations = []  # Reset for each trial
        
        # keep track of which components have finished
        TrialComponents = [trial_fixation_cross, trial_ABA_sequence, trial_key_resp]
        ...
        while continueRoutine:
            ...
            # *trial_fixation_cross* updates
            
            # if trial_fixation_cross is starting this frame...
            if trial_fixation_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                trial_fixation_cross.frameNStart = frameN  # exact frame index
                trial_fixation_cross.tStart = t  # local t and not account for scr refresh
                trial_fixation_cross.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(trial_fixation_cross, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'trial_fixation_cross.started')
                # update status
                trial_fixation_cross.status = STARTED
                trial_fixation_cross.setAutoDraw(True)
            
            # if trial_fixation_cross is active this frame...
            if trial_fixation_cross.status == STARTED:
                # update params
                pass
            
            # if trial_ABA_sequence is starting this frame...
            if trial_ABA_sequence.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                trial_ABA_sequence.frameNStart = frameN  # exact frame index
                trial_ABA_sequence.tStart = t  # local t and not account for scr refresh
                trial_ABA_sequence.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('trial_ABA_sequence.started', tThisFlipGlobal)
                # update status
                trial_ABA_sequence.status = STARTED
                trial_ABA_sequence.play(when=win)  # sync with win flip
            
            # if trial_ABA_sequence is stopping this frame...
            if trial_ABA_sequence.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trial_ABA_sequence.tStartRefresh + 7.0-frameTolerance:
                    # keep track of stop time/frame for later
                    trial_ABA_sequence.tStop = t  # not accounting for scr refresh
                    trial_ABA_sequence.tStopRefresh = tThisFlipGlobal  # on global time
                    trial_ABA_sequence.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'trial_ABA_sequence.stopped')
                    # update status
                    trial_ABA_sequence.status = FINISHED
                    trial_ABA_sequence.stop()
            # update trial_ABA_sequence status according to whether it's playing
            if trial_ABA_sequence.isPlaying:
                trial_ABA_sequence.status = STARTED
            elif trial_ABA_sequence.isFinished:
                trial_ABA_sequence.status = FINISHED
            ...
            if trial_key_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trial_key_resp.tStartRefresh + 7-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if trial_key_resp.status == STARTED and not waitOnFlip:
                theseKeys = trial_key_resp.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # Run 'Each Frame' code from trial_multiple_keypress
            # Each Frame tab
            ...
            for key, timestamp in keys:
                if key == 'space':
                    if press_start_time is None:
                        ...
                        press_start_time = None  # Reset for the next press
        ...
        trial_ABA_sequence.pause()  # ensure sound has stopped at end of Routine
        # check responses
        ...
        # Run 'End Routine' code from trial_multiple_keypress
        # End Routine tab
        thisExp.addData('response_times', response_times)
        thisExp.addData('press_durations', press_durations)
    ...
    # keep track of which components have finished
    BreakComponents = [break_message, break_key_resp]
    ...
    while continueRoutine:
        ...
        if break_key_resp.status == STARTED and not waitOnFlip:
            theseKeys = break_key_resp.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
    ...
    if break_key_resp.keys != None:  # we had a response
        ...
        thisExp.addData('break_key_resp.duration', break_key_resp.duration)
    thisExp.nextEntry()
